...
class KafkaWriter[Key_Type, Message_Type](
  kafka_conf: Config,
  ...
  val writer_future_pool_size = kafka_conf.getInt("writer_future_pool_size")

  val producerConfig = new ProducerConfig(props);
  val producer = new Producer[Key_Type, Message_Type](producerConfig);
  val kafkaWriterPool = FuturePool(
    Executors.newFixedThreadPool(
      writer_future_pool_size,
      new NamedPoolThreadFactory("KafkaWriter")
    )
  )
